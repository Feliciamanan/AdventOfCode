/*
  ...
  Example:      - ADVENT contains no markers and decompresses to itself with no
                ...
                capabilities, allowing many-gigabyte files to be stored in only
                a few kilobytes.
                ...
                enough memory to actually decompress the file; you'll have to
...
int64_t getUncompressedLength(const string &input, int sIndex, int eIndex) {
  // cout << "\t\tuncompressing: ";
  // for (int i = sIndex ; i < eIndex; ++i){
  //   cout << input[i];
  // }
  // cout << endl;
  int64_t result = eIndex - sIndex;
  int letters = 0;
  int times = 0;
  int startIndex = 0;
  for (int i = sIndex ; i < eIndex; ++i ) {
    switch(input[i]){
      ...
      case ')':
        if (letters && times){
          cout << "\t" << letters << "x" << times << " From " << startIndex;
          cout << " to " << i;
          ...
          int64_t expandingLine = getUncompressedLength(input,i+1,i+1+letters);
          int64_t expanded = expandingLine * (times);
          cout << "\t\tExpanding Line from " << i+1 << " to " << i+1+letters;
          cout << ". is: " << expandingLine << " multiplied " << times;
          cout << " = " << expanded << endl;
          result += expanded;
          result -= (expressionLen + letters);
          i += letters;
        }
        break;
...
int64_t getUncompressedLength(const string &input) {
